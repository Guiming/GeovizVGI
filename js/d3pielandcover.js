// chart dimensions
var width_pie_landcover = 400;
var height_pie_landcover = 300;
...
var color_landcover = d3.scale.ordinal()
                    .domain(["Evergreen needleleaf forests",
...
function draw_piechart_landcover(dataset, update){

  if(/*true == update*/d3.select("#svg_pie_landcover")[0][0] != null){
    d3.select("#svg_pie_landcover").remove();
  }
  if(dataset.length == 0){
    return;
  }
  svg_pie_landcover = d3.select('#d3pielandcover') // select element in the DOM with id 'chart'
    ...
    .attr('transform', 'translate(' + (width_pie_landcover / 3) + ',' + (height_pie_landcover / 2) + ')'); // our reference is now to the 'g' element. centerting the 'g' element to the svg element
  ...
  // define tooltip
  var tooltip = d3.select('#d3pielandcover') // select element in the DOM with id 'chart'
    .append('div') // append a div element to the element we've selected
    .attr('class', 'tooltiplandcover'); // add class 'tooltip' on the divs we just selected
  ...
  var legend = svg_pie_landcover.selectAll('.legendlandcover') // selecting elements with class 'legend'
    ...
    .attr('transform', function(d, i) {
      ...
      var horz = 10 * legendRectSize_landcover; // the legend is shifted to the left to make room for the text
